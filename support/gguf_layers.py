...
def get_layer_count(path):
    ...
    print("Failed to find *.block_count in light GGUF parse; trying GGUFReader…")
    ...
    try:
        ...
        if layer_count is not None:
            if isinstance(layer_count, (list, tuple)):
                return int(layer_count[0])
            try:
                # gguf may return ndarray / field parts
                return int(layer_count[0])
